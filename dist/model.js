...
export function selectShows(shows, {mode='now',query='',venue='',today=todayNY()}={}) {
 ...
 .filter(s=>!venue||s.venue===venue)
 .filter(s=>[s.title,s.credits,s.description||'',s.venue,s.neighborhood,...s.types].join(' ').toLowerCase().includes(query.trim().toLowerCase()))
